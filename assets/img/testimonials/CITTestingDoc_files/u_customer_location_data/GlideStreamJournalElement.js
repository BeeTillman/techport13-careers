...
var GlideStreamJournalElement = Class.create({
	initialize: function(name) {
		...
		CustomEvent.observe('sn.stream.loaded', function() {
			this.streamLoaded = true;
			if (this.fireToggleWhenLoaded)
	...
	isVisible : function(ge, control) {
		var isHidden = false;
		var fieldObj = $j('#activity-stream-' + this.fieldname + '-textarea').closest('.form-group');
		if (fieldObj) 
			isHidden = fieldObj.css('display') !== 'none' && fieldObj.css('visibility') !== 'hidden';
		return isHidden;
	},
	...
	showFieldMsg: function(input, message, type) {
		if (this.streamLoaded)
			...
			this.fireToggleWhenLoaded = true;
		CustomEvent.fire('sn.form.journal_field.show_msg', this.fieldname, message, type);
	},
	hideFieldMsg: function(input, clearAll) {
		CustomEvent.fire('sn.form.journal_field.hide_msg', this.fieldname, clearAll);
	},
