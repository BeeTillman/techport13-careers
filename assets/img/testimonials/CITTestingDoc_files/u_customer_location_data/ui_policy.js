...
GlideFieldPolicy.prototype = {
	...
	runPolicyOnLoad: function() {
		if (this.fp.onload && typeof g_form !== "undefined")
			this.runPolicy();
	},
	runPolicy: function() {
		var category = this.source_table === 'sys_data_policy2' ? 'sys_data_policy2' : 'ui_policy';
		CustomEvent.fire('glide_optics_inspect_put_context', category, '"'+this.description+'"', this.sys_id, this.source_table);
	...
	runDeferredListPolicy: function() {
		this._runListActions(this.lastResult);
	},
	...
	hasLisActions: function() {
		return this.fp.hasOwnProperty("list_actions") && this.fp.list_actions.length;
	},
...
if (term.oper == "DATEPART")
		...
		switch (term.oper) {
			...
			case '<=':
				...
				return (valueDate != 0) && (term.oper === '<=' ? userDate <= valueDate : userDate < valueDate);
			case '>':
			case '>=':
				var valueDate = getDateFromFormat(value, g_user_date_time_format);
				return (valueDate != 0) && (term.oper === '>=' ? userDate >= valueDate : userDate > valueDate);
			...
			case 'BETWEEN':
				...
				return (valueDate1 != 0) && (valueDate2 != 0) && (userDate >= valueDate1) && (userDate <= valueDate2);
			case 'LESSTHAN':
				return this._dateComparisonHelper(term.field, term.value, 'LT', userFormat);
			case 'MORETHAN':
				return this._dateComparisonHelper(term.field, term.value, 'GT', userFormat);
			default:
				this.debug("evaluateTermDate - unsupported operator '" + term.oper + "'. Returning FALSE.");
				return false;
		}
	},
	_dateComparisonHelper: function(left, right, comparison, userFormat) {
		...
		var userDate = getDateFromFormat(userValue, userFormat);
		var theDate = getDateFromFormat(value, userFormat);
if (userDate == 0 || theDate == 0) {
			this.debug("evaluateTermDate - invalid date, returning false");
			return false;
		}
	...
	_getAbsQuarter: function(theDate) {
		...
		var Q = quarters[thisDate.getMonth()];
		var Y = thisDate.getFullYear() * 4;
		return Y+Q;
	},
	_roundDateToQuarter: function(theDate) {
		...
		var Q = quarters[thisDate.getMonth()];
		var d = new Date();
		d.setFullYear(thisDate.getFullYear(), Q, 0);
		d.setHours(0,0,0);
		return d;
	},
...
var userValue = g_form.getValue(term.field + ".storage") + '';
	...
	runActions: function(result) {
		...
		if (this.fp.onLoad && window.g_related_list_timing === 'deferred') return;
		
		this._runListActions(result);
	},
	
	_runListActions: function(result) {
	...
	_runAction: function(action, result) {
		...
		var mandatory = action.mandatory;
		var visible = action.visible;
		var disabled = action.disabled;
		var cleared = action.cleared;
		var mappingField = this._getVariableMappingField(field);
		if (mappingField) {
			var uiPolicyActions = {
				mandatory: mandatory,
	...
	_parseDurationValue: function(value) {
		...
		var answer = valueParts.shift() + ' ' + valueParts.join(':');
		if (answer === "0 00:00:00")
			answer = "00 00:00:00";
		return answer;
	},
...
function ui_policy_onLoad() {
	...
	if(g_ui_policy && g_ui_policy.length)
		for (var i = 0; i < g_ui_policy.length; i++)	{
			...
			if (fp.hasLisActions() && window.g_related_list_timing === 'deferred')
				uiDeferredListPolicies.push(fp);
			uiPolicies.push(fp);
			var __ptmr = new Date();
...
CustomEvent.observe('deferred_lists_loading_complete', function() {
	if (uiDeferredListPolicies && uiDeferredListPolicies.length) {
		var sw =  new StopWatch();
		for (var i = 0; i < uiDeferredListPolicies.length; i++)	{
			uiDeferredListPolicies[i].runDeferredListPolicy();
		}
		sw.jslog("ui_policy deferred_list loaded");
	}
});
