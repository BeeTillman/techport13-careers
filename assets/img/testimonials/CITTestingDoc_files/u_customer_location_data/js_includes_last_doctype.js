...
function addTextInput(td, dValue, type, associatedField, label) {
	...
	var msg = NOW.msg;
	var defaultLabel = label ? msg.getMessage(label) : msg.getMessage('Input value');
...
function addConditionSpec(name, queryID, field, oper, value, fDiv) {
	...
	if (NOW && NOW.msg && window.NOW && window.NOW.accessibility && window.NOW.accessibility.ariaLivePolite) {
		...
		if(isAndCondition) {
			...
			window.NOW.accessibility.ariaLivePolite(
				m.getMessage("New AND condition added, {0} of {0}", conditionsCount), 0);
		}
		else {
...
tr    += "</h3></td></tr>";
	...
	addCondition: function(wantOR, field, oper, value) {
		...
		this.conditions[this.conditions.length] = condition;
		value = this.unescapeQueryValue(oper,value);
	...
	unescapeQueryValue: function(oper, val) {
		if (!val)
			return val;
		if (oper === "MATCH_RGX")
val = val.replace(/\^\^/gm, "^");
...
this.value = value.replace(/&amp;/g,'&');
	...
	addNewSubCondition: function(field, oper, value) {
		...
		this.value = this.unescapeQueryValue(oper, value);
	...
	unescapeQueryValue: function (oper, val) {
		if (this.section)
			return this.section.unescapeQueryValue(oper, val);
	},		
...
td.style.paddingBottom = "4px";
	...
	build: function(field, oper, value) {
		...
		this.fieldSelect.parentNode.scrollIntoView({behavior: "smooth", block: "end", inline: "nearest"});
		return tds;
	},
