...
getQuery: function( options) {
	...
	getRelatedQuery: function() {
		...
		var collectionKey = this.getSubmitValue('sysparm_collection_key');
		if (!collectionKey)
			return null;
		return collectionKey + "=" + this.getSubmitValue('sysparm_collectionID');
	},
...
if (this.isHierarchical())
			GwtListEditor.forPage._prepareTable(this.table, "dblclick");
		this.setUpAriaControlsForToggles();
	...
	setUpAriaControlsForToggles: function() {
		var hierarchyToggles = $(this.listID).querySelectorAll('img[data-type="list2_hier"], i[data-type="list2_hier"]');
		for (var i = 0; i < hierarchyToggles.length; i++) {
			var row = this._getRowRecord(hierarchyToggles[i]);
			hierarchyToggles[i].setAttribute('aria-controls', 'hier_row_' + this.listID + '_' + row.sysId)
		}
	},
	...
	onHdrCellClick: function(element, ev) {
		...
		var focusFn = function() {
			...
			if (document.querySelector('#show_list')) {
				var hamEles = listEle.querySelectorAll('i.list_header_context.list-column-icon');
				Array.prototype.forEach.call(hamEles, function(node) {
					node.parentNode.removeChild(node);
				});
			}
			
			var anchor = listEle.querySelector('[name="'+field+'"] a[data-type="list2_hdrcell"].list_hdrcell');
			if (anchor) {
				anchor.focus();
			}
		...
		GlideUXMetrics && GlideUXMetrics.track('Sort Column', {
			name: field,
...
return false;
		...
		variables['g_fieldType'] = element.getAttribute('glide_type');
		variables['g_sysId'] = '';
		variables['rowSysId'] = '';
		this.headerMenu.setOnShow(this._getOnShowScript());
		this.headerMenu.setOnHide(this._getOnHideScript());
		this.headerMenu.showContextMenu(getEvent(ev), "context_list_header", variables);
		return false;
	},
	_getOnShowScript: function() {
		return function() {
			var firingObj = this.getFiringObject();
			if (firingObj)
				firingObj.setAttribute('aria-expanded', true);
		}
	},
	_getOnHideScript: function() {
		return function() {
			var firingObj = this.getFiringObject();
			if (firingObj)
				firingObj.removeAttribute('aria-expanded');
		}
	},
	...
	getFormView: function() {	
		...
		return url.getParam('sysparm_form_view') || '';	
	},
	getView: function() {
		return this.view;
	},
...
setFilter: function(filter,  saveOrderBy,  saveGroupBy) {
	...
	setDefaultFilter: function(filter, includeOrderBy) {
		includeOrderBy = includeOrderBy === true;
		this.setFilter(filter, includeOrderBy);
...
this.fieldNames.push('');
	...
	setOmitCount: function(omitCount) {
	...
	toggleListNoPref: function() {
		...
		if(NOW.isUsingPolaris) {
			toggleDivDisplayClass(collapsedID);
			var listDiv = toggleDivDisplayClass(expandedID);
		} else {
		...
		var isHidden = getComputedStyle(listDiv).display === "none";
		if (isHidden) {
...
this.listID = this.listID + '_' + colmn;
	...
	_getGroupToggler: function() {
		var elements = this.listDiv.select('.list_group_toggle');
	...
	_toggleGroup: function(table, row) {
		...
		CustomEvent.fire('reclone_header');
...
img.src = "images/list_v2_heir_hide.gifx";
	...
	toggleHierarchy: function(img, rowId, parentTable, parentSysId) {
		...
		if (row.getAttribute("hierarchical") == "not_loaded") {
			...
			this._sendAriaLiveMessage(new GwtMessage().getMessage("Hierarchical List Loaded"));
			GlideListUxMetricsUtils.track('Heiarchical List Toggled', this, {
				opened: true,
				recordId: parentSysId
			});
			return;
		}
		var shown = row.getAttribute("collapsed") != "true";
		...
		GlideListUxMetricsUtils.track('Toggle Heiarchical List', this, {
			opened: shown,
			recordId: parentSysId
		});
		CustomEvent.fire('list.section.toggle', this.table, this);
	...
	loadList: function(el, parentTable, parentSysId, template, listCss) {
		...
		ajax.addParam('escapePhaseTwo', 'true');
...
action: function(actionId, actionName,  allowedCheckedIds) {
	...
	_action: function(actionId, actionName, ids, isClientSide) {
		...
		if (this.getView() === 'sys_ref_list' && actionName === 'sysverb_new')
			this.addToForm('sysparm_view', this.getFormView());
		else
...
_refreshPage: function( firstRow,  additionalParms, includeOrderBy) {
		...
		url.addParam(this._createFirstRowKey(), this.firstRow);
		url.addParam('sysparm_view', this.view);
		var q = this.submitValues['sysparm_fixed_query'];
		if (q)
			url.addParam('sysparm_fixed_query', q);
		if (this.groupBy.length > 0 && !!this.groupSort)
			url.addParam('sysparm_group_sort', this.groupSort);
...
_refreshAjax: function( firstRow,  additionalParms, includeOrderBy) {
		...
		if (this.groupBy.length > 0 && !!this.groupSort)
			ajax.addParam('sysparm_group_sort', this.groupSort);
		
		if (this.isReport) {
			ajax.addParam('sysparm_report_list', this.isReport);
		}
		var firstRowKey = this._createFirstRowKey();
		ajax.addParam(firstRowKey, this.firstRow);
		ajax.addParam('stackparm_' + firstRowKey, this.firstRow);
        ajax.addParam('sysparm_properties', this.properties);
		ajax.addParam('sysparm_refresh', 'true');
		if (typeof g_ck != 'undefined' && g_ck != "")
			ajax.addParam('sysparm_ck', g_ck);
		ajax.addParam('sys_hint_nocache', 'true');
		ajax.addParam('sysparm_stack', 'no');
	...
	setGroupSort: function(dir) {
		dir = !!dir ? dir : "";
		this.groupSort = dir;
		this._refreshPage(1, null, true);
	},
	_sort: function(field, dir) {
		...
		this._refreshAjax(1, parms, true);
		var reloadEventName = 'partial.page.reload';
		var listID = this.listID;
		var focusFn = function() {
	...
	_setRowCounts: function() {
		...
		if(skippedRowsSpan) {
			...
			if(this.groupBy.length === 0 && this.totalRows <= this.rowsPerPage) {
				this.totalRows -= this.skippedRows;
				this.lastRow = this.totalRows;
			}
		}
		var doesNextPageExist = (this.rowsPerPage + 1) === this.totalRows;
		var totalRows = this.totalRows;
		if (this.omitCount && doesNextPageExist)
		...
		if (this.totalRows == 0)
			...
			e.innerHTML = new GwtMessage().getMessage("{0} ({1})", this.getTitle(), totalRows);
	},
	_clear: function() {
		this.lastChecked = null;
		this.table = null;
...
_parseQuery: function(queryString,  saveOrderBy,  saveGroupBy) {
		...
		for (var i = 0; i < q.length; i++) {
			...
			if (term.indexOf("sys_tags=") == 0)
term = term.replace(/(sys_tags)=(.*)/i, '$1.$2=$2');
	...
	_separateIntoTerms: function(queryString) {
		...
		while (queryString.substring(startIndex).length > 0) {
            ...
            if (separatorIndex < queryString.length - 1) {
                while (queryString.charAt(separatorIndex + 1) == '^' && separatorIndex !== -1) {
...
term = term.replace(/\^\^/g, "^");
	...
	_validateFirstRow: function(row) {
		...
		row = parseInt(row, 10);
		if (!this.omitCount) {
			row = this._validateRowPosition(row);
		}
		if (row < 1)
			row = 1;
		return row;
	},
	_validateRowPosition: function(row) {
		if (row > this.totalRows)
			return (this.totalRows - this.rowsPerPage) + 1;
		return row;
	},
	...
	listMechanicClick: function(element) {
		...
		GlideListUxMetricsUtils.track('Open Personalize List Columns Modal', this, {
			parentId: window.g_list_parent_id,
			listParent: window.g_list_parent,
			listView: window.g_list_view,
			relationship: window.g_list_relationship
		});
...
if (this.tableName !== table)
	...
	_fireListReady: function() {
		if (!this.table){
			var nlqErrorStateCaptionMessage = this._getNLQErrorStateLiveMessage();
			this._sendAriaLiveMessage(nlqErrorStateCaptionMessage);
			return;
		};
		if (!this.isRelated || this.announceOnReady) {
			var showingRecordsMessage = this._generateShowingRecordsLiveMessage();
			var referenceCaptionMessage = this._getReferenceCaptionLiveMessage();
			this._sendAriaLiveMessage(referenceCaptionMessage + " " + showingRecordsMessage);
		}
	...
	_getReferenceCaptionLiveMessage: function() {
		var listElement = document.getElementById(this.listID);
		var listCaptionElement = listElement.querySelector('#reference-list-caption');
		if (!listCaptionElement) return '';
		return listCaptionElement.innerText;
	},
	_getNLQErrorStateLiveMessage: function() {
		var listElement = document.getElementById(this.listID);
		var listCaptionNLQElement = listElement.querySelector('#nlq-error-state-caption');
		if (!listCaptionNLQElement) return '';
		return listCaptionNLQElement.innerText;
	},
	_generateShowingRecordsLiveMessage: function() {
	...
	_createFirstRowKey: function() {
		if (this.isRelated) {
			var parentSysId = this.getSubmitValue('sysparm_collectionID');
			return 'sysparm_first_row_' + this.parentTable + "_" + parentSysId + '_' + this.tableName;
		}
		return 'sysparm_first_row';
	},
	type: 'GlideList2'
});
...
var GlideWidgetVCR = Class.create(GlideListWidget, {
	...
	initialize: function($super, widgetID, listID, omitCount) {
		...
		this.navigationMap = {'back': "Previous", 'first': 'First', 'next': 'Next', 'last': 'Last'};
	},
	gotoAction: function(ev, el) {
		...
		GlideUXMetrics && GlideUXMetrics.track('Select Page', { page: this.navigationMap[action] });
...
row = row && row.replace(/,| |\./g, '');
		...
		list._refreshAjax(row, {}, true);
		this._restoreFocusAfterPartialReload(el);
		GlideUXMetrics && GlideUXMetrics.track('Select Page', { page: 'Page' + list.firstRow });
	},
...
_refresh: function(listTable,  list) {
		if (!this.span || this.span.innerHTML == "")
			this.span = $(this.widgetID + "_vcr");
		if (!this.omitCount && list.totalRows == 0) {
			...
			this.doesNextPageExist = (list.rowsPerPage + 1 === list.totalRows);
			this.nextAllowed = this.omitCount ? this.doesNextPageExist : (list.lastRow < list.totalRows);
		...
		if (row)
			row.disabled = this.omitCount ? true : (list.totalRows <= list.rowsPerPage);
	},
	_$: function(suffix) {
	...
	_setRowNumbers: function(list) {
		var lastRow = this._$('_last_row'),
			...
			nextPageRowCount = this.doesNextPageExist ? list.rowsPerPage : list.grandTotalRows,
			omitCountTotalRows = list.firstRow + nextPageRowCount - 1;
		if (lastRow)
			lastRow.innerHTML = this.omitCount ? omitCountTotalRows.toLocaleString() : list.lastRow.toLocaleString();
		if (totalRows)
			totalRows.innerHTML = list.totalRows.toLocaleString();
		if (firstRow)
			firstRow.value = list.firstRow.toLocaleString();
        if (descriptionOfSkipTo) {
        	if (this.omitCount) {
				getMessage("Showing rows {0} to {1}", function (msg) {
					descriptionOfSkipTo.innerHTML = new GwtMessage().format(msg, list.firstRow.toLocaleString(), omitCountTotalRows.toLocaleString());
				});
        	} else {
		...
		this._setButtonsClickable(list);
	},
	_setButtonsClickable : function(list) {
		var firstPage = this._$('_first'),
			previousPage = this._$("_back"),
			nextPage = this._$("_next"),
			lastPage = this._$("_last");
		var isBackDisabled = list.firstRow === 1,
			isNextDisabled = this.omitCount ? !this.doesNextPageExist : list.lastRow === list.totalRows;
		previousPage.disabled = isBackDisabled;
		firstPage.disabled = isBackDisabled;
		nextPage.disabled = isNextDisabled;
		lastPage.disabled = this.omitCount ? true : isNextDisabled;
	},
...
if (!flag)
	...
	_restoreFocusAfterPartialReload: function(elToFocus) {
		...
		var listDiv = GlideList2.get(this.listID).listDiv;
		var focusFn = function() {
			if (elToFocus.disabled)
				elToFocus = elToFocus.parentElement.querySelector('[id$="_first_row"]');
			elToFocus.focus();
			window.scrollTo(0,0);
...
_refresh: function(listTable,  list) {
	...
	actionCheck: function(select, ignoreSessionMessages) {
		...
		if (actions.length > 0) {
			...
			this._canExecute(actionIds, sysIds, list, ignoreSessionMessages);
			for (var i = 0; i < actions.length; i++) {
				var opt = actions[i];
		...
		if ( getTopWindow().g_list_choice_action_dedupe == 'true' && options.length > 0) {
			for (var i = 0; i < options.length; i++) {
				...
				if (this._shouldHide(opt, select)) {
					...
					var wrapper = new Element('span', {'display': 'none'});
...
_checkAction: function(opt,  sysIds) {
	...
	_canExecute: function(actionIds, sysIds, list, ignoreSessionMessages) {
		var wantSessionMessages = !ignoreSessionMessages;
		var ajax = new GlideAjax("AJAXActionSecurity");
		...
		ajax.setWantSessionMessages(wantSessionMessages);
		var xml = ajax.getXMLWait();
		var answer = {};
	...
	_trackMetrics: function(opt, list) {
		if (!opt || !list)
			return;
		var actionName = opt.getAttribute('gsft_base_label');
		var isClientSide = getAttributeValue(opt, 'client') == 'true';
		var payload = {
			actionName: actionName,
			isClientSide: isClientSide
		};
		if (isClientSide) {
			var closest = opt.closest('optgroup');
			var path = closest
				? closest.getAttribute('label') + ' > ' + actionName
				: actionName;
			payload.path = path;
			payload.selectedRecordsCount = list.getChecked().split(',').length;
		}
		else {
			var ids = opt.getAttribute('gsft_allow') || list.getChecked();
			payload.selectedRecordsCount = ids.split(',').length;
		}
		GlideListUxMetricsUtils.track("Apply List Action", list, payload);
	},
...
_refresh: function(listTable,  list,  isInitialLoad) {
		...
		if (this.focusOnRefresh) {
			...
			if (this.accessibilityEnabled) {
				var table = GlideList2.get(listTable).table;
				var shouldFocusTable = table.getAttribute('data-search-show') == 'true';
				if (shouldFocusTable)
					e = $j('#' + this.listID + '_table');
				else
			...
			top.NOW.magellan && top.NOW.magellan.invokeWhenShouldFocus &&
...
var a = text.nextSibling;
	...
	search: function(ev) {
		...
		var parms = {};
		parms['sysparm_goto_query'] = this._escapeQueryValue(value);
	...
	_escapeQueryValue: function(val) {
		if (!val)
			return val;
...
function runFilterV2Lists(name, filter, listId) {
	var list = GlideList2.get(listId || name);
	if (!list) {
	...
	if (list) {
	    ...
	    if (groupBy) 
	        filter += "^" + groupBy;
		GlideListUxMetricsUtils.track("Run Filter", list, {
			"query": filter,
			"cbType": "jelly"
		});
...
GlideList2.saveFilter = function(listID, listName) {
    ...
    if (groupBy) 
        filter += "^" + groupBy;
	GlideListUxMetricsUtils.track("Save Filter", list, {
		"query": filter,
...
GlideList2.setDefaultFilter = function(listID, listName, includeOrderBy) {
    var filter = getFilter(listName, false);
	var list = GlideList2.get(listID);
	GlideListUxMetricsUtils.track("Set Default Filter", list, {
		"query": filter,
		"includeOrderBy": includeOrderBy,
		"cbType": "jelly"
	});
    GlideList2.get(listID).setDefaultFilter(filter, includeOrderBy);
}
;
...
if (evt && (evt.keyCode == 32 || evt.keyCode == 13 && isMSIE11))
	...
	document.body.on('click', 'ul[data-type="group_sort_option"] a', function(evt, element) {
		if (element.getAttribute("data-is-selected") == "true") {
			evt.stop();
			return;
		}
		evt.preventDefault();
		var groupSort = element.getAttribute("data-group_sort");
		GlideList2.get(element.getAttribute('data-list_id')).setGroupSort(groupSort);
	});
...
if (evt.ctrlKey)
	...
	document.body.on('keypress', 'a[data-type="list2_toggle"]', function (evt, element) {
if (evt.keyCode !== 32 )
			return;
		GlideList2.get(element.getAttribute('data-list_id')).toggleList();
		evt.preventDefault();
	...
	if(isDoctype()) {
		var checkboxClickHandler = function(e) {
			...
			var listId = GlideList2.get(input.getAttribute('data-list_id'));
			listId.rowChecked(input, e);
			$j(input).change();
			if (input.checked)
				$j(input).parents('tr').addClass('is-selected');
			else {
				$j(input).parents('tr').removeClass('is-selected');
				$j('input[data-type="list2_all_checkbox"]').removeClass('is-selected');
			}
			var tar = $j(input).parents('tr')[0];
			if (tar) {
				var options = {
					table: listId.tableName,
					sysId: input.dataset.uxMetricsSysid,
					action: input.checked ? 'select' : 'unselect'
				}
				GlideUXMetrics && GlideUXMetrics.track('Select Record', options);
			}
		};
		$j('div.list_v2').on('click', 'input[data-type="list2_checkbox"] + label.checkbox-label', checkboxClickHandler);
		$j('div.list_v2').on('keydown', 'input[data-type="list2_checkbox"]', checkboxClickHandler);
	} else {
	...
	document.body.on('click', 'input[data-type="list2_all_checkbox"]', function (evt, element) {
		...
		if (element.checked) {
			$j(element).addClass('is-selected');
			$j(element).parents('table').find('input[data-type="list2_checkbox"]').parents('tr').addClass('is-selected');
		} else {
			$j(element).removeClass('is-selected');
			$j('input[data-type="list2_checkbox"]').parents('tr').removeClass('is-selected');
		}
		evt.stopPropagation();
	});
...
toggleIcon.className = toggleIcon.className.replace(/\bcollapsedGroup\b/,'');
	...
	document.on('click', 'img[data-type="list2_popup"], a[data-type="list2_popup"]', function (evt, element) {
		...
		GlideUXMetrics && GlideUXMetrics.track('Preview Record',options);
	});
	if (isDoctype()) {
...
if (evt && (evt.keyCode == 32 || evt.keyCode == 13 && window.isMSIE11))
	...
	document.body.on('keypress', 'a[data-type="list_mechanic2_open"], i[data-type="list_mechanic2_open"]', function (evt, element) {
if (evt.keyCode !== 32 )
			return;
		GlideList2.get(element.getAttribute('data-list_id')).listMechanicClick(element);
		evt.preventDefault();
	...
	window.g_isGlideList2InitEventsFinished = true;
}
...
;
/*! RESOURCE: /scripts/classes/GlideListUxMetricsUtils.js */
var GlideListUxMetricsUtils = {
	track: function(eventName, list, payload) {
		if (!GlideUXMetrics || typeof (list) === 'undefined'|| !list || !eventName)
			return;
		var newPayload = Object.assign({
			"interface-type": "CoreUI",
			"tableName": list.getTableName(),
			"parentTable": list.parentTable,
			"listId": list.listID,
			"listName": list.listName,
			"query": list.getQuery({all: true}),
			"isHierarchical": list.isHierarchical()
		}, payload);
		if (newPayload.isHierarchical) {
			newPayload.hierachicalData = {
				relatedListName: $(list.listID + '_table').getAttribute('hier_list_name'),
				relatedListTitle: list.title,
				recordId: $(list.listID + '_table').up('tr.list_hierarchical_hdr').previous().getAttribute('sys_id')
			};
		}
		GlideUXMetrics.track(eventName, newPayload);
	}
}; 
;
...
CustomEvent.observe('list2_init', function(list2) {
	...
	function cloneHeader() {
		...
		var $pageHeader = $j('.list_nav_top.list_nav[role="region"]').first();
		var pageHeaderHeight = $pageHeader.outerHeight();
		if (!$pageHeader.hasClass('hierarchical')) {
			...
			var navbarMarginTop = $pageHeader.outerHeight() + parseInt($pageHeader.css('padding-bottom')) - parseInt($pageHeader.css('border-bottom-width'));
		...
		var $firstSearchInput = $j($header).find('tr.list_header_search_row input.list_header_search:first');
		$firstSearchInput.removeAttr('id');
		...
		$cloneTable.find('[tabindex],button,a').each(function (i, elem) {$j(elem).attr('tabindex', -1).attr('aria-hidden', true);});
		...
		$cloneTable.find('script').remove();
		$cloneTable.find('thead').find('tr').find('th').each(function() {
...
var listHeaderSpacerHeight = $j('.list_nav_spacer').offset();
	...
	CustomEvent.observe('reclone_header', reclone);
...
var orderByMatches = query.match(/\^ORDERBY/g) !== null ? query.match(/\^ORDERBY/g).length : 0;
			...
			for (var i = 0; i < parts.length; i++) {
				...
				var isBoolean = $table.find('tr.list_header_search_row td[name="'+p.field+'"]').attr("data-glide-type") === "boolean";
				if (isBoolean) {
					if(value === "1")
						value = getMessage("True");
					else if(value === "0")
						value = getMessage("False");
					else
				...
				if (p.Goto == 'true')
					hasGotoQuery = true;
				if (setInputValue($table, p.field, buildDisplayValue(p.operator, unescapeQueryValue(p.operator,value))))
	...
	function getQueryFromTable($table, tableName) {
		...
		$table.find('tr.list_header_search_row td').each(function(index, el) {
			...
			if ($input.length === 0)
				return;
			var value = $input.val() || '';
			var isBoolean = el.getAttribute('data-glide-type') === "boolean";
			var newTerm;
			var choiceQuery = $input.data('choice_query');
			if (choiceQuery)
				newTerm = choiceQuery;
			else
				newTerm = buildTerm(field, value, isBoolean);
			var found = false;
			...
			if (!found && value)
				terms.push(newTerm);
			GlideUXMetrics && GlideUXMetrics.track('Apply Column Filter', {
				name: field,
				table: tableName,
				fieldType: type
			});
		...
		for (var i = 0; i < terms.length; i++) {
			...
			if (!t.field)
				continue;
			query += '^' + t.field + t.operator + escapeQueryValue(t.value);
		}
		return query;
	}
	function buildTerm(field, value, isBoolean) {
		var operator = isBoolean ? '=' : 'STARTSWITH';
		var trueValue;
		...
		if (isBoolean) {
  			operator = ["=", "!=", "IN"].indexOf(operator) > -1 ? operator : "IN";
  			if (value !== "" && ["0", "1"].indexOf(value) < 0 ) {
	 			trueValue = getMessage("True");
 				value = trueValue.toLowerCase().startsWith(value.toLowerCase()) ? "True" : "False";
			}
  		}
		return {
			field: field,
...
return val.replace(/\^\^/gm, "^").replace(/\^/gm, "^^");
	...
	function unescapeQueryValue(oper, val) {
		if (!val)
			return val;
		if (oper === "MATCH_RGX")
val = val.replace(/\^\^/gm, "^");
return val.replace(/\^\^+/gm, function(value) {
...
var GlideWidgetFilter = Class.create(GlideListWidget, {
	...
	togglePin: function() {
		...
		GlideListUxMetricsUtils.track("Pin Filter", this._getList(), {
...
if (!bc_hidden.innerHTML)
	...
	_filterDisplay: function(showFlag) {
		var e = this._getFilterDiv();
		var f = gel(this.listID + "nlqfilterdiv");
		var g = gel(this.listID + "nlqfilterTipsdiv");
		if (!e)
			return;
...
e.src = "images/list_v2_filter_reveal.gifx";
	...
	_loadFilter: function(targetDiv, listDiv) {
	       this._filterDisplay(true);
	       targetDiv.setAttribute('gsft_empty', 'false');
	       var isExcludedRole = 'true';
	       var disableSorting = 'false';
	       if (listDiv) {
		   isExcludedRole = listDiv.getAttribute('data-is-excluded-role');
		   disableSorting = listDiv.getAttribute('data-disable-sorting');
		}
	       ...
	       ajax.addParam("sysparm_view", list.getView());
	       ajax.addParam("sysparm_is_excluded_role", isExcludedRole);
	       ajax.addParam("sysparm_disable_sort", disableSorting);
	       try {
	...
	_getFilterDiv: function() {
		return gel(this.listID + "filterdiv");
	},
	_getListDiv: function() {
    		return gel(this.listID + "_list");
    },
...
if (ev.type === 'keypress' && ev.keyCode !== 32 )
	...
	_trackOpenFilterMetrics: function() {
		if (!this._isFilterEnabled)
			return;
		var e = this._getFilterDiv();
		if (!e)
			return;
		var isGsftEmpty = e.getAttribute('gsft_empty') == 'true';
		var showFlag = isGsftEmpty ||  e.style.display == "none";
		GlideListUxMetricsUtils.track("Launch Advance Filter", this._getList(), {
			"showFlag": showFlag,
			"cbType": "jelly"
		});
	},
	...
	_initBreadcrumbEvents: function(span) {
		...
		span.on('click', 'a.breadcrumb_separator', this._runBreadcrumbSeparator.bind(this));
		span.on('click', 'a.breadcrumb_link', this._runBreadcrumbLink.bind(this));
		span.on('click', 'button.breadcrumb_separator', this._runBreadcrumbSeparator.bind(this));
		span.on('click', 'button.breadcrumb_link', this._runBreadcrumbLink.bind(this));
		...
		span.on('keydown', 'a.breadcrumb_link', this._onKeydownBreadcrumbContext.bind(this));
		span.on('keydown', 'button.breadcrumb_link', this._onKeydownBreadcrumbContext.bind(this));
	},
	_runBreadcrumbSeparator: function(evt) {
		this._runBreadcrumb(evt, "Remove Next Breadcrumb");
	},
	_runBreadcrumbLink: function(evt) {
		this._runBreadcrumb(evt, "Remove Subsequent Breadcrumb");
	},
	_onKeydownBreadcrumbContext: function(evt) {
		if ((evt.shiftKey && evt.keyCode === 32) || (evt.altKey && evt.keyCode === 121)) {
			this._onBreadcrumbContext(evt);
		}
	},
	...
	_runBreadcrumb: function(evt, eventName) {
		...
		GlideListUxMetricsUtils.track(eventName, this._getList(), {
			"query": filter,
...
return;
		...
		var isSysRelationship = list.getRelated() && !list.getRelatedQuery();
		if (isSysRelationship) {
crumbMenu.addLabel(msgs.getMessage("URLs/query omit relationship to parent record"));
			crumbMenu.addLine();
		}
		var that = this;
		var newWindowItem = crumbMenu.addURL(msgs.getMessage('Open new window'), link, "_blank", "open_new");
		newWindowItem.onclick = function() {
			GlideListUxMetricsUtils.track("Apply Breadcrumb Action", that._getList(), {
				"action": "Open new window",
				"cbType": "jelly"
			});
		}
var baseURL = new URL(document.baseURI || document.URL).origin + "/";
		var copyURLItem = crumbMenu.addFunc(msgs.getMessage('Copy URL'), function() {
			GlideListUxMetricsUtils.track("Apply Breadcrumb Action", that._getList(), {
				"action": "Copy URL",
				"cbType": "jelly"
			});
...
$j(function($) {
	...
	function updateFormElement(id, dateString) {
		...
		if (element_calendar && element_timeago) {
			...
			element_calendar.setAttribute('title', dateString);
...
return $.trim(this.tagInput.val().replace(/^"(.*)"$/, '$1'));
		...
		createTag: function(value, additionalClass, duringInitialization, labelJson) {
			...
			if (!this.options.allowDuplicates && !this._isNew(value)) {
				...
				if (existingTag.data('uncommon')) {
					existingTag.find('.tagit-label:first').css('font-style', '');
					this.tagInput.val('');
...
var label = $((this.options.onTagClicked || g_enhanced_activated == 'true')? '<a style="color:' + labelJson.tcolor + '" class="tagit-label"></a>' : '<span class="tagit-label"></span>')
				.attr('aria-label', new GwtMessage().getMessage('View records tagged with {0}', value))
				.attr('title', new GwtMessage().getMessage('View records tagged with {0}', value))
				.text(displayValue);
			...
			if (!this.options.query) {
				...
				label.click(function(){
					var list = GlideList2.get(that.options.table) || GlideList2.get(that.element.context);
...
var removeTag = $('<a tabindex="0" role="button"><span style="color:' + labelJson.tcolor + '" class="text-icon icon-cross"></span></a>')
...
tag.addClass('removed');
		...
		removeTagById: function(tag, id, animate) {
			...
			var tags = this.assignedTags();
			var removedTagLabel = this.tagLabel(tag);
			var removeTagIndex = tags.indexOf(removedTagLabel);
			if (this.options.singleField) {
				tags = $.grep(tags, function(el){
					return el != removedTagLabel;
				});
...
tag.addClass('removed');
			...
			this._trigger('afterTagRemoved', null, {tag: id, tagLabel: removedTagLabel, table:this.options.table,
				rowId:this.options.rowId});
			tags = this.assignedTags();
			if (tags.length > removeTagIndex) {
				$("#" + tags[removeTagIndex]).focus();
			} else {
				this.tagInput.focus();
			}
		},
...
;
/*! RESOURCE: /scripts/GlideWidgetNLQFilter.js */
var GlideWidgetNLQFilter = Class.create(GlideListWidget, {
	initialize: function($super, widgetID, listID, listName, query, pinned, saveFilterHidden, nlqAppendQueryEnabled, nlqOpenOnLoad) {
		$super(widgetID, listID);
		this.query = query;
		this.listName = listName;
		this.checkFilterEnabled();
		this.pinned = (pinned == 'true');
		this.openOnRefresh = nlqOpenOnLoad == 'true';
		this.nlqAppendQueryEnabled = nlqAppendQueryEnabled;
		if (!saveFilterHidden)
			this.saveFilterHidden = false;
		else
			this.saveFilterHidden = saveFilterHidden;
	...
	toggleFilter: function() {
		if (!this._isFilterEnabled)
			return;
		var filterDiv = this._getFilterDiv();
		if (!filterDiv)
			return;
		if (filterDiv.getAttribute('gsft_empty') == 'true') {
			this._loadNLQFilter(filterDiv);
			return;
		}
		if (filterDiv.querySelector('.list_filter_row sn-nlq-query-input')) {
			setTimeout( function() {
				filterDiv.querySelector('.list_filter_row sn-nlq-query-input').shadowRoot.querySelector('sn-nlq-input').shadowRoot.querySelector('now-typeahead').focus();
			}, 0);
		}
		var showFlag = filterDiv.style.display == "none";
		this._filterDisplay(showFlag);
	},
_refresh: function(listTable,  list, loadFlag) {
		if (loadFlag) {
	...
	_filterDisplay: function(showFlag) {
		var e = this._getFilterDiv();
		var tipsLinkElement = this._getFilterTipsLink(this.listID);
		var f = gel(this.listID + "filterdiv");
		if (!e)
			return;
		if (showFlag) {
			showObjectInlineBlock(e);
			showObjectInlineBlock(tipsLinkElement);
			if (f)
				hideObject(f);
		}
		else {
			hideObject(e);
			hideObject(tipsLinkElement);
		}
		e = gel(this.listID + "_filter_toggle_nlq_image");
		if (!e)
			return;
		writeTitle(e, this._getFilterIconMessage());
		CustomEvent.fire('list.section.toggle');
	...
	_getFilterIconMessage: function() {
return getMessage('Show / hide natural language filter');
	},
	_loadNLQFilter: function(targetDiv) {
		this._filterDisplay(true);
		targetDiv.setAttribute('gsft_empty', 'false');
		var list = this._getList();
		var ajax = new GlideAjax("AJAXJellyRunner", "AJAXJellyRunner.do");
		ajax.addParam("template", "list2_filter_nlq_partial.xml");
		ajax.addParam("sysparm_widget_id", this.widgetID);
		ajax.addParam("sysparm_list_id", this.listID);
		ajax.addParam("sysparm_list_name", this.listName);
		ajax.addParam("sysparm_table", list.getTableName());
	...
	_getFilterDiv: function() {
		return gel(this.listID + "nlqfilterdiv");
	},
	_getFilterTipsLink: function(listID) {
		return gel(listID + "nlqfilterTipsdiv");
	},
	checkFilterEnabled: function() {
		var list = this._getList();
		this._isFilterEnabled = list.isFilterEnabled();
	...
	_getFilterToggle: function(list) {
		list = list || this._getList();
		var filterToggleId = '#' + list.listID + '_filter_toggle_nlq_image';
		return list.listContainer.select(filterToggleId);
	},
	_initEvents: function(list) {
		var button = this._getFilterToggle(list);
		var link = this._getFilterTipsLink(list.listID);
		if (button.length == 1) {
			if (!this._isFilterEnabled) {
				$j(button[0])
					.attr('aria-disabled', 'true')
				...
				this._initFilterEvents(button[0]);
		}
		if (link) {
			this._initTipsLinkEvents(link);
		}
		CustomEventManager.observe('list-nlq-callback', this.nlqCallback.bind(this));
	},
	nlqCallback: function(payload) {
		if (!payload || !payload.apiResult || !payload.apiResult.data_configurations)
			return;
		var apiResult = payload.apiResult;
		var dataConfiguration =  apiResult.data_configurations;
		if (!Array.isArray(dataConfiguration)) return;
		var listNlqConfig = apiResult.data_configurations.find(function(config){return config.type === 'list' || config.type === 'bar'});
		var list = this._getList();
		var url = new GlideURL(list.getTableName() + "_list.do");
		GlideListUxMetricsUtils.track('Submit Natural Language Filter', list, payload);
		if (!listNlqConfig){
			url.addParam('sysparm_nlq_error_state', 'true');
			url.addParam('sysparm_previous_query', this.query);
			top.NOW.magellan && top.NOW.magellan.setNavigatingState && top.NOW.magellan.setNavigatingState(true);
			window.g_navigation.open(url.getURL());
		}
		var nlqResponseQuery = listNlqConfig.filter_query;
		var groupByField = listNlqConfig.group_by;
		if (groupByField)
			nlqResponseQuery = nlqResponseQuery + 'GROUPBY' + groupByField;
		if (this.nlqAppendQueryEnabled.toLowerCase() === 'true') {
			list.addFilter(nlqResponseQuery);
			url.addParam('sysparm_query', list.getQuery({orderby: true}));
		}
		else {
			url.addParam('sysparm_query', nlqResponseQuery);
		}
		url.addParam('sysparm_view', list.getView());
		var q = list.getFixedQuery();
		if (q)
			url.addParam('sysparm_fixed_query', q);
		var css = list.getSubmitValue('sysparm_list_css');
		if (css)
			url.addParam('sysparm_list_css', css);
		var queryNoDomain = list.getSubmitValue('sysparm_query_no_domain');
		if (queryNoDomain)
			url.addParam('sysparm_query_no_domain', queryNoDomain);
		url.addParam('sysparm_nlq_query', true);
		top.NOW.magellan && top.NOW.magellan.setNavigatingState && top.NOW.magellan.setNavigatingState(true);
		window.g_navigation.open(url.getURL());
	},
	_initFilterEvents: function(button) {
		var self = this;
		$j(button).attr('aria-expanded',self.pinned);
		$j(button).on('click', function(ev) {
			var $this = $j(this);
			self.toggleFilter();
			var isExpanded = $this.attr('aria-expanded') === 'true';
			var opened = !isExpanded;
			$this.attr('aria-expanded',opened);
			ev.preventDefault();
			var list = self._getList();
			GlideListUxMetricsUtils.track('Open Natural Language Filter', list, {
				opened: opened
			});
		});
	},
	_initTipsLinkEvents: function(link) {
		var self = this;
		$j(link).on('click', function(ev) {
			self.tipsLinkOnClick();
			ev.preventDefault();
		});
	},
	_getList: function() {
	...
	tipsLinkOnClick: function(e) {
		var dialog = new GlideModal('nlq_help_modal');
		dialog.setTitle(new GwtMessage().getMessage('Tips to improve results'));
		dialog.render();
		dialog.setPreference('focusTrap', true);
	},
	goBackToPreviousPage: function(previouseQuery) {
		var list = this._getList();
		var queryLink = list.listName + "_list.do?sysparm_query=" + encodeURIComponent(previouseQuery);
		var url = new GlideURL(queryLink);
		url.setEncode(false);
		url.addParam('sysparm_view', this._getList().getView());
		var link = url.getURL();
		top.NOW.magellan && top.NOW.magellan.setNavigatingState && top.NOW.magellan.setNavigatingState(true);
		window.g_navigation.open(link);
	},
	type: 'GlideWidgetNLQFilter'
});
;
;
/*! RESOURCE: /scripts/js_includes_list_nlq.js */
/*! RESOURCE: /scripts/classes/GlideListUxMetricsUtils.js */
var GlideListUxMetricsUtils = {
	track: function(eventName, list, payload) {
		if (!GlideUXMetrics || typeof (list) === 'undefined'|| !list || !eventName)
