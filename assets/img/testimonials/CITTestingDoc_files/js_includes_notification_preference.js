	...
	return {
		...
		controller: function($scope, $element, snCustomEvent, notificationPreferencesService, systemProperties, i18n) {
			...
			scrollContainer.bind('scroll', function() {
				...
				if ((pageIndex*pageSize) < notificationPreferencesService.totalCount) {
					pageIndex++;
					loadPreferences();
				}
			});
			...
			$scope.openEditRecord = function openEditRecord($event) {
				...
				if (!$scope.record.sys_id) {
					...
					var formParams = {
						...
						'^user=' + (($window.NOW.user_id) ? $window.NOW.user_id : $window.NOW.user.userID)
					};
				}
		...
		controller: function($scope, snCustomEvent, snNotification, i18n, viewStackService,
			...
			function loadChannels() {
				...
				notificationChannelsService.load().then(function(channels) {
					$scope.globalActive = notificationChannelsService.globalActive;
					$scope.enableNotificationsReadonly = notificationChannelsService.enableNotificationsReadonly;
					$scope.enableNotificationsReadonlyReason = notificationChannelsService.enableNotificationsReadonlyReason;
	...
	return {
		...
		link: function(scope, element) {
			...
			frame.on('load', function() {
				...
				if (frameHead) {
					var isPolaris = frameDoc.body.className.indexOf('-polaris') > -1;
					...
					if (!isPolaris) {
						...
						customStyles.append('body[data-formname=sys_form_template] form .ng-filter-widget { min-width: auto; background-color: #e7e9eb; width: calc(100% + 69px) !important; overflow: visible; }');
					}
					...
					customStyles.append('div#element\\.sys_notif_subscription\\.condition > div.col-xs-10.filter_controls { width: 100%; } ');
...
if (firstFocusable.length)
				...
				this.contentWindow.CustomEvent.observe("iframe_form.sysverb_cancel", iframeFormCancelled);	
...
var baseUrl = '/api/now/v1/notification/';
	...
	var _enable_notifications_readonly = true;
	var _enable_notifications_readonly_reason = '';
	return {
		load: function load(options) {
			var self = this;
...
url += '/' + options.channel;
			...
			return snResource().get(url).then(function success(response) {
				...
				if (options.channel) {
					...
					_enable_notifications_readonly = response.data.result.enable_notifications_readonly;
					_enable_notifications_readonly_reason = response.data.result.enable_notifications_readonly_reason;
...
snResource().post(baseUrl + 'preference/channel', {
		...
		get enableNotificationsReadonly() {
			return _enable_notifications_readonly;
		},
		
		get enableNotificationsReadonlyReason() {
			return _enable_notifications_readonly_reason;
		},
